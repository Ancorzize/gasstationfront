...
export const CashWithdrawalModal = ({ isOpen, onClose, cashSessions, summary, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await cashService.registerWithdrawal({
      ...
      if (res.status) {
        showToast(res.message || "Retiro registrado correctamente.", "success");
        onSave();
        onClose();
        setFormData({ caja_id: '', monto: '', medio_pago: 'efectivo', descripcion: '' });
      } else {
        showToast(res.message || "La caja no tiene saldo suficiente.", "error");
      }
    } catch (error) {
      showToast("Error de conexión al registrar retiro", "error");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-[2.5rem] w-full max-w-lg p-6 md:p-8 shadow-2xl border border-slate-100 space-y-6">
        <div className="flex justify-between items-center">
          <h3 className="text-sm font-black text-slate-800 uppercase flex items-center gap-2">
            <ArrowDownCircle className="text-red-500" size={20} /> Retiro Manual de Caja
          </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? <Loader2 className="animate-spin" size={16} /> : "Guardar Retiro"}
          </button>
        </form>
      </div>
    </div>
