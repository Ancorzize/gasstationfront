  ...
  return (
    <div className="p-4 md:p-8 space-y-6 max-w-6xl mx-auto pb-20">
      ...
      <div className="sticky top-0 z-40 bg-slate-50/90 dark:bg-zinc-950/90 backdrop-blur-md pt-2 pb-4 space-y-4 -mx-4 px-4 md:mx-0 md:px-0">
        <header className="flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
              Estación: {selectedTurno.estacion?.nombre} | Islero:{" "}
              {selectedTurno.usuario?.name}
            </p>
          </div>
        </header>
        ...
        {/* Banner de Balance */}
        <div
          className={`p-5 rounded-[2rem] shadow-md border flex items-center justify-between transition-colors ${balance === 0 ? "bg-emerald-50 border-emerald-100 text-emerald-900" : balance < 0 ? "bg-rose-50 border-rose-100 text-rose-900" : "bg-blue-50 border-blue-100 text-blue-900"}`}
        >
          <div>
            ...
            <p className="text-[9px] md:text-[10px] font-bold opacity-75">
              Esperado: {formatPesos(totalEsperado)} | Reportado:{" "}
              {formatPesos(totalReportado)}
            </p>
          </div>
          <div className="text-right">
            <p className="text-sm md:text-xl font-black">
              {balance >= 0 ? "Sobrante" : "Faltante"}:{" "}
              {formatPesos(Math.abs(balance))}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {selectedTurno.lecturas && selectedTurno.lecturas.length > 0 ? (
              selectedTurno.lecturas.map((l) => {
                ...
                const nombreProducto =
                  l.manguera?.producto?.nombre || "Combustible";
                ...
                return (
                  <div
                    key={l.id || l.manguera_id}
                    className="mb-4 p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2"
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <span className="text-xs font-black text-slate-900 block">
                          {formatPesos(totalVenta)}
                        </span>
                        <span className="text-[9px] font-bold text-emerald-600">
                          {galones.toFixed(3)} gal
                        </span>
                      </div>
                    </div>
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase text-center py-6">
                No hay registros de lecturas para este turno.
              </p>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            {selectedTurno.recaudos &&
              selectedTurno.recaudos.map((destino) => (
                <div
                  key={destino.id}
                ...
                >
                  <h3 className="text-xs font-black uppercase mb-6 flex items-center gap-2 text-slate-800">
                    <Banknote size={16} />{" "}
                    {destino.destino_recaudo?.nombre || "Recaudo"}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        readOnly
                        disabled
                        className="w-full p-3 rounded-xl text-xs font-black text-right outline-none bg-slate-100 border border-slate-200 text-slate-700 cursor-not-allowed"
                        value={formatPesos(destino.efectivo)}
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] font-bold text-slate-400 uppercase">
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        disabled
                        className="w-full p-3 rounded-xl text-xs font-black text-right outline-none bg-slate-100 border border-slate-200 text-slate-700 cursor-not-allowed"
                        value={formatPesos(destino.datafono)}
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] font-bold text-slate-400 uppercase">
                      ...
                      <input
                        ...
                        disabled
                        className="w-full p-3 rounded-xl text-xs font-black text-right outline-none bg-slate-100 border border-slate-200 text-slate-700 cursor-not-allowed"
                        value={formatPesos(destino.transferencia)}
                      />
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 p-6 md:p-8 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-slate-50 rounded-xl space-y-1 border border-slate-100">
                  ...
                  <span className="text-xs font-black text-slate-800">
                    {formatPesos(selectedTurno.otros_movimientos)}
                  </span>
                </div>
                <div className="p-3 bg-slate-50 rounded-xl space-y-1 border border-slate-100">
                  <span className="text-[9px] font-bold text-slate-400 uppercase block">
                  ...
                  <span className="text-xs font-bold text-slate-800">
                    {selectedTurno.otros_movimientos_detalle || "N/A"}
                  </span>
                </div>
              </div>
              ...
              <div className="p-3 bg-slate-50 rounded-xl space-y-1 border border-slate-100">
                ...
                <p className="text-xs font-bold text-slate-700">
                  {selectedTurno.observacion_cierre ||
...
export const IsleroMisTurnosPage = () => {
  ...
  const fetchMisTurnos = async () => {
    ...
    try {
      ...
      const response = await shiftService.getIsleroTurns({
        estado: "cerrado",
        user_id: userId,
      });
                ...
                {filteredTurnos.map((turno) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-slate-500">
                      {turno.fecha_cierre
                        ? new Date(turno.fecha_cierre).toLocaleString()
                        : "N/A"}
                    </td>
                    <td className="p-4 font-black text-slate-900">
