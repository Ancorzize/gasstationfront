...
export const ShiftManagementPage = () => {
  ...
  const canVenderCombustible = hasPermission("vender_combustible");
  const canVenderLubricantes = hasPermission("vender_lubricantes");
  const canRegistrarAbonos = hasPermission("registrar_abonos_cartera");
  const hasAnyQuickAction = canVenderCombustible || canVenderLubricantes || canRegistrarAbonos;
          ...
          {!currentShift ? (
            <div className="bg-white rounded-[2.5rem] border border-slate-100 p-8 md:p-12 text-center max-w-xl mx-auto space-y-6 shadow-sm">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium px-2 mt-1">
                  Para comenzar a registrar ventas de combustible y lubricantes,
                  debes abrir un turno en tu estación asignada.
                </p>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 space-y-6">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-3 mt-2 text-zinc-400 text-[9px] md:text-[10px] font-bold uppercase tracking-widest">
                        <span className="flex items-center gap-1">
                          <MapPin size={12} /> {currentShift.estacion?.ciudad}
                        </span>
                        <span className="flex items-center gap-1">
                          <User size={12} /> {currentShift.user?.name}
                        </span>
                      </div>
                    </div>
                  </div>
                  <Fuel
                    className="absolute -right-6 -bottom-6 text-white opacity-5 pointer-events-none"
                    size={160}
                  />
                </div>
                ...
                <div className="bg-white rounded-[2rem] border border-slate-100 p-6 shadow-sm">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {canVenderCombustible && (
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between">
                              <span className="text-xs md:text-sm font-black text-slate-800 uppercase">
                                Venta Combustible
                              </span>
                              <ArrowRight
                                size={16}
                        ...
                        {canRegistrarAbonos && (
                          <button
                            ...
                            className={`p-5 bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 border border-emerald-500/20 rounded-2xl text-left hover:border-emerald-500/40 transition-all group ${
                              (!canVenderCombustible || !canVenderLubricantes) ? 'sm:col-span-1' : 'sm:col-span-2'
                            }`}
                          >
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-100">
                    <button
                      onClick={() =>
                        navigate(`/turnos-islero/${currentShift.id}/resumen`)
                      }
              ...
              <div className="bg-white rounded-[2rem] border border-slate-100 p-6 h-fit">
                <h4 className="text-xs font-black text-slate-800 uppercase mb-4">
                ...
                <div className="space-y-3">
                  {currentShift.lecturas?.map((l) => (
                    <div
                      key={l.id}
                      className="flex items-center justify-between p-3.5 bg-slate-50 rounded-xl border border-slate-100/50"
                    >
                      <div>
