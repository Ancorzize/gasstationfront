...
export const CustomerStatementPage = () => {
  ...
  const handleRegisterInitialDebt = async (e) => {
    ...
    try {
      ...
      if (res.status) {
        showToast(
          res.message || "Deuda inicial registrada correctamente.",
          "success",
        );
  ...
  const { cliente, movimientos, cupo_credito, saldo_credito, cupo_disponible } =
    data;
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {hasPermission("crear_saldos_iniciales_cartera") && (
            ...
            >
              <PlusCircle size={18} className="text-blue-600" /> Registrar deuda
              inicial
            </button>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-xl font-black text-slate-800">
              $ {Number(cupo_credito || 0).toLocaleString()}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-xl font-black text-slate-800">
              $ {Number(saldo_credito || 0).toLocaleString()}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-xl font-black text-slate-800">
              $ {Number(cupo_disponible || 0).toLocaleString()}
            </p>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-50">
              {movimientos?.length > 0 ? (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      ...
                      <p className="text-[9px] font-bold text-slate-400 uppercase">
                        {mov.medio_pago || "CRÉDITO"}
                      </p>
                    </td>
                    <td
                      className={`px-6 py-4 text-right font-black text-xs ${mov.tipo_movimiento === "abono" ? "text-emerald-600" : "text-red-600"}`}
                    >
                      {mov.tipo_movimiento === "abono" ? "-" : "+"} $
                      {Number(mov.valor || 0).toLocaleString()}
                    </td>
                ...
                <tr>
                  <td
                    colSpan="6"
                    className="px-6 py-20 text-center text-[10px] font-black text-slate-300 uppercase italic"
                  >
        ...
        {isInitialDebtModalOpen && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            >
              ...
              <form
                onSubmit={handleRegisterInitialDebt}
                id="form-initial-debt"
                className="space-y-4"
              >
